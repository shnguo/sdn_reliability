...
def verification(G,pos,final_pos,assigment,assigment_nodetocontroller):
    ...
    for node in G.nodes():
        if shortest_path_matrix[node][assigment_nodetocontroller[node]]>min_max:
            min_max_p=shortest_path_matrix[node][assigment_nodetocontroller[node]]
    ...
    return min_max_pro,min_max_list,min_max_dict,end-start

def main():
    g,pos=bmm.build_map_mat()
    final_pos,assigment,assigment_nodetocontroller=cp.calculate_pro(g,pos,1)
    A,B,C,D=verification(g,pos,final_pos,assigment,assigment_nodetocontroller)
    print(A,B,C,D)
